                                        ...
                                        <div class="tab-content" id="v-pills-tabContent">
                                                        ...
                                                        <div id="avatar_container">
                                                            <div class="collapseOne card-collapse collapse @if($user->profile->avatar_status == 0) show @endif">
                                                                <div class="card-body text-center">
                                                                    <img src="{{ asset('images/avatar.png') }}" alt="{{ $user->name }}" class="user-avatar rounded-circle" style="width: 120px; height: 120px; object-fit: cover;">
                                                                </div>
                                                            </div>
                                                            <div class="collapseTwo card-collapse collapse @if($user->profile->avatar_status == 1) show @endif">
                                                                <div class="card-body text-center">
                                                                    ...
                                                                    {!! csrf_field() !!}
                                                                            <img id="user_selected_avatar" class="user-avatar rounded-circle" src="@if ($user->profile->avatar != NULL) {{ $user->profile->avatar }} @endif" alt="{{ $user->name }}" style="width: 120px; height: 120px; object-fit: cover;">
                                                                    {!! Form::close() !!}
                                                                </div>
                                                            </div>
                                                        </div>
                                            ...
                                            <div class="tab-pane fade edit-settings-tab" role="tabpanel" aria-labelledby="edit-settings-tab">
                                                {!! Form::model($user, array('action' => array('ProfilesController@updateUserAccount', $user->id), 'method' => 'PUT', 'id' => 'user_basics_form')) !!}
                                                    ...
                                                    <div class="pr-3 pl-2 form-group has-feedback row {{ $errors->has('phone_number') ? ' has-error ' : '' }}">
                                                        {!! Form::label('phone_number', 'Teléfono', array('class' => 'col-md-3 control-label')); !!}
                                                        <div class="col-md-9">
                                                            <div class="input-group">
                                                                {!! Form::text('phone_number', $user->phone_number, array('id' => 'phone_number', 'class' => 'form-control', 'placeholder' => 'Ingrese el número de teléfono')) !!}
                                                                <div class="input-group-append">
                                                    ...
                                                    <div class="pr-3 pl-2 form-group has-feedback row {{ $errors->has('softland_user') ? ' has-error ' : '' }}">
                                                        {!! Form::label('softland_user', 'Usuario Softland', array('class' => 'col-md-3 control-label')); !!}
                                                        <div class="col-md-9">
                                                            <div class="input-group">
                                                                <select class="form-control" name="softland_user" id="softland_user">
                                                                    <option value="">Seleccione un usuario Softland</option>
                                                                    @if ($usuariosSoftland)
                                                                        @foreach($usuariosSoftland as $usuario)
                                                                            <option value="{{ $usuario->USUARIO }}" {{ $user->softland_user == $usuario->USUARIO ? 'selected="selected"' : '' }}>{{ $usuario->USUARIO }} - {{ $usuario->NOMBRE }}</option>
                                                                        @endforeach
                                                                    @endif
                                                                </select>
                                                                <div class="input-group-append">
                                                                    <label class="input-group-text" for="softland_user">
                                                                        <i class="fa fa-fw fa-user" aria-hidden="true"></i>
                                                                    </label>
                                                                </div>
                                                            </div>
                                                            @if ($errors->has('softland_user'))
                                                                <span class="help-block">
                                                                    <strong>{{ $errors->first('softland_user') }}</strong>
                                                                </span>
                                                            @endif
                                                        </div>
                                                    </div>

                                                    <div class="pr-3 pl-2 form-group has-feedback row {{ $errors->has('softland_store') ? ' has-error ' : '' }}">
                                                        {!! Form::label('softland_store', 'Bodega Softland', array('class' => 'col-md-3 control-label')); !!}
                                                        <div class="col-md-9">
                                                            <div class="input-group">
                                                                <select class="form-control" name="softland_store" id="softland_store">
                                                                    <option value="">Seleccione una bodega</option>
                                                                    @if ($bodegas)
                                                                        @foreach($bodegas as $bodega)
                                                                            <option value="{{ $bodega->BODEGA }}" {{ $user->softland_store == $bodega->BODEGA ? 'selected="selected"' : '' }}>{{ $bodega->BODEGA }} - {{ $bodega->NOMBRE }}</option>
                                                                        @endforeach
                                                                    @endif
                                                                </select>
                                                                <div class="input-group-append">
                                                                    <label class="input-group-text" for="softland_store">
                                                                        <i class="fa fa-fw fa-warehouse" aria-hidden="true"></i>
                                                                    </label>
                                                                </div>
                                                            </div>
                                                            @if ($errors->has('softland_store'))
                                                                <span class="help-block">
                                                                    <strong>{{ $errors->first('softland_store') }}</strong>
                                                                </span>
                                                            @endif
                                                        </div>
                                                    </div>
...
@section('footer_scripts')
    ...
    <script type="text/javascript">
        ...
        // Plugin de medidor de fortaleza de contraseña no disponible
        // $('#password').password({
